...
/**
 * Class UserGroupPermissionsController
 *
...
class UserGroupPermissionsController extends UserMgmtAppController {
	...
	public function index() {
		...
		if ($c > -2) {
			...
			foreach ($user_group_permissions as $row) {
				...
				$userGroupName = $row['UserGroup']['alias_name'];
				$allowed = $row['UserGroupPermission']['allowed'];
				$con[$cont][$act][$userGroupName] = $allowed;
			}
	...
	public function update() {
		...
		$controller = $paramData['controller'];
		$action = $paramData['action'];
		$result = $this->UserGroup->getGroupNamesAndIds();
		$success = 0;
		foreach ($result as $row) {
			if (isset($paramData[$row['alias_name']])) {
				...
				if (empty($res)) {
					...
					$data['UserGroupPermission']['allowed'] = $paramData[$row['alias_name']];
					...
					if ($paramData[$row['alias_name']] != $res['UserGroupPermission']['allowed']) {
						$data = array();
						$data['UserGroupPermission']['allowed'] = $paramData[$row['alias_name']];
